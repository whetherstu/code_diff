...
# this dictionary stores the predecessors and successors of nodes//节点的前驱和后继
...
#angr分析文件一般是二进制文件，通常对于汇编文件来说是将汇编文件先转为二进制文件
def angrGraphGen(filepath1, filepath2):
    ...
    prog1 = prog1.asm(filepath1)
    prog2 = prog2.asm(filepath2)
...
# 这个函数的主要目的是计算两个二进制文件中不同类型指令的数量以及它们的索引映射。这对于后续生成特征向量或进行二进制分析等任务可能会有用
...
# 这个函数的主要目的是计算两个二进制文件中不同类型字符串引用的数量，以及生成偏移量和字符串引用的映射。它还记录不属于相应二进制文件的函数名，
...
filepath1 = "E:\\big\\subtrain\\0ACDbR5M3ZhBJajygTuf.asm"
filepath2 = "E:\\big\\subtrain\\0AnoOZDNbPXIr2MRBSCJ.asm"
cfg1, cg1, nodelist1, edgelist1, cfg2, cg2, nodelist2, edgelist2 = angrGraphGen(filepath1, filepath2)
print(cfg1)
